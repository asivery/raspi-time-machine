...
from config_objects import AdditionalEvent, Module, Field
from config import CONFIG_ROOT, HOST, PORT
...
CONFIG = join(CONFIG_ROOT, "time-machine.json")
...
DATE = None
DATE_TOLERANCE = None
DATE_TOLERANCE_THRESHOLD_DISABLED = None
GEOCITIES_FIX = None
QUICK_IMAGES = None
WAYBACK_API = None
CONTENT_TYPE_ENCODING = None
INJECT_AUTO_RELOAD = None

INTERNAL_IP = "192.168.2.1"

def update_system_configuration(_):
    try:
        check_output(["bash", "./modules/time_machine/prepare.sh"])
    except: pass


class TimeMachine(Module):
    def __init__(self, dispatcher):
        super().__init__("Time Machine", dispatcher)
        self.load_config()
        Thread(target=start_proxy).start()
        update_system_configuration(None)
        dispatcher.register_handler("network_changed", update_system_configuration)
    
    def get_fields(self):
        def handle_partial_date_update(source, value):
            self.update([1], value)
            self.dispatcher.handle_event(Event("update_value", [source, self.name, 1]))
        return [
            Field("", "label", ("This module is heavily based on WaybackProxy by richardg867", "Org. richardg867")),
            Field("Date", "date", DATE[:4] + "-" + DATE[4:6] + "-" + DATE[6:], {
                AdditionalEvent.partial_update: handle_partial_date_update
            }),
            Field(("Date Tolerance (in days)", "Date Tlrnc Days"), "int", DATE_TOLERANCE),
            Field(("Disable Date Tolerance Threshold", "Date Tlr.Th off"), "bool", DATE_TOLERANCE_THRESHOLD_DISABLED),
            Field("Geocities Fix", "bool", GEOCITIES_FIX),
            Field("Quick Images", "bool", QUICK_IMAGES),
            Field("Use Wayback API", "bool", WAYBACK_API),
            Field(("Content Type Encoding", "Content Type Enc."), "bool", CONTENT_TYPE_ENCODING),
            Field(("Inject auto-reload script", "Inj. Auto reload"), "bool", INJECT_AUTO_RELOAD)
        ]

    def update(self, path, value):
        global DATE, DATE_TOLERANCE, GEOCITIES_FIX, QUICK_IMAGES, WAYBACK_API, CONTENT_TYPE_ENCODING, DATE_TOLERANCE_THRESHOLD_DISABLED, INJECT_AUTO_RELOAD
        path = path[0]
        if path == 1:
            DATE = value.replace("-", "")
        elif path == 2:
            DATE_TOLERANCE = int(value)
        elif path == 3:
            DATE_TOLERANCE_THRESHOLD_DISABLED = value
        elif path == 4:
            GEOCITIES_FIX = value
        elif path == 5:
            QUICK_IMAGES = value
            ...
            CONTENT_TYPE_ENCODING = value
        elif path == 8:
            INJECT_AUTO_RELOAD = value
        self.save_config()
    ...
    def load_config(self):
        ...
        if not exists(CONFIG):
            with open(CONFIG, 'w') as e:
                e.write("{}")
        with open(CONFIG, 'r') as e:
            content = json.load(e)
            DATE = content.get("date", "19961113")
            DATE_TOLERANCE = content.get("dateTolerance", 365)
            GEOCITIES_FIX = content.get("geocitiesFix", True)
            QUICK_IMAGES = content.get("quickImages", True)
            WAYBACK_API = content.get("waybackApi", True)
            CONTENT_TYPE_ENCODING = content.get("contentTypeEncoding", True)
            DATE_TOLERANCE_THRESHOLD_DISABLED = content.get("disableDateToleranceThreshold", False)
            INJECT_AUTO_RELOAD = content.get("injectAutoReload", True)

    def save_config(self):
        obj = {
            "date": DATE,
            "dateTolerance": DATE_TOLERANCE,
            "geocitiesFix": GEOCITIES_FIX,
            "quickImages": QUICK_IMAGES,
            "waybackApi": WAYBACK_API,
            "contentTypeEncoding": CONTENT_TYPE_ENCODING,
            "disableDateToleranceThreshold": DATE_TOLERANCE_THRESHOLD_DISABLED,
            "injectAutoReload": INJECT_AUTO_RELOAD,
        }
        with open(CONFIG, 'w') as e:
            json.dump(obj, e)
...
# Modified waybackproxy.py from richardg867's WaybackProxy on github (https://github.com/richardg867/WaybackProxy) below

import base64, datetime, json, re, socket, socketserver, string, sys, threading, traceback, urllib.request, urllib.error, urllib.parse
import modules.time_machine.lrudict as lrudict
...
class Handler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        try:
            ...
            # Start fetching the URL and make sure the method is "GET" unless it's a loopback to the config page.
            if method != "GET" and hostname != "time.machi.ne":
                return self.send_error_page(http_version, 501, 'Not Implemented')
            contentData = None
            if method != "GET":
                if content_length == -1:
                    return self.send_error_page(http_version, 400, 'Invalid request')
                contentData = bytes(f.read(content_length), "utf-8")
            conn = urllib.request.urlopen(urllib.request.Request(request_url, method=method), data=contentData)
        ...
        if 'text/html' in guessed_content_type:
            ...
            if DATE_TOLERANCE != None and not DATE_TOLERANCE_THRESHOLD_DISABLED:
            ...
            if mode == 0: # Wayback Machine
                ...
                # If needed, inject a script that will automatically reload the page once
                # any configuration parameter changes (this is a special file handled by 
                # webeditor.py, and is tied to the update_value event
                if INJECT_AUTO_RELOAD:
                    data = re.sub(b'''</body>''', bytes(f'''<script src="http://{INTERNAL_IP}:{PORT}/reload.js"></script></body>''', 'ascii'), data)
